...
function ustream_shortcode( $atts ) {
	...
	$output = sprintf(
		'<iframe assets="%1$s" width="%2$d" height="%3$d" scrolling="no" style="border: 0 none transparent;"></iframe>',
		esc_url( $url ),
		absint( $atts['width'] ),
		absint( $atts['height'] )
	);
...
function ustreamsocial_shortcode( $atts ) {
	...
	return sprintf(
		'<iframe id="SocialStream" assets="%1$s" class="" name="SocialStream" width="%2$d" height="%3$d" scrolling="no" allowtransparency="true" style="visibility: visible; margin-top: 0; margin-bottom: 0; border: 0;"></iframe>',
		esc_url( $url ),
		absint( $atts['width'] ),
		absint( $atts['height'] )
	);
